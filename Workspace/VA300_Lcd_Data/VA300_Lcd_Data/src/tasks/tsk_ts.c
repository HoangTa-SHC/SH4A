		...
		if ( ercd == E_OK ) {
			...
			if ( ( ( MdGetMode() == MD_INITIAL ) && ( ( GetScreenNo() == LCD_SCREEN6 ) || ( GetScreenNo() == LCD_SCREEN8 ) ) )
			  ...
			  || ( ( MdGetMode() == MD_MAINTE )  && ( ( GetScreenNo() == LCD_SCREEN203 ) ) ) ){
				...
				if ( ( GetScreenNo() == LCD_SCREEN6 ) || ( GetScreenNo() == LCD_SCREEN127 ) ){ //　画面が「指をセットして」なら
					wait_screen();					// 画面が「指を抜いて下さい」または、失敗画面になるのを待つ。
				}

				dly_tsk( 2000/MSEC );				// 最低2秒間は、「指を抜いて下さい」画面を保持。
//				dly_tsk( 1000/MSEC );				// 最低１秒間は、「指を抜いて下さい」画面を保持。
				...
				if ( ( MdGetMode() == MD_INITIAL ) && ( GetScreenNo() == LCD_SCREEN7 ) ){ // 初期モード（登録）「指を抜いて下さい」
					...
					ercd = set_flg( ID_FLG_LCD, FPTN_LCD_SCREEN8 );	// 認証完了画面へ。	
					if ( ercd == E_OK ){
				...
				if ( ( MdGetMode() == MD_NORMAL )  && ( GetScreenNo() == LCD_SCREEN128 ) ){ // 通常モード（登録）「指を抜いて下さい」
					while ( check_touch_off() != E_OK ){ // 生体検知センサーのオフ・タッチを検出する
				  		dly_tsk( 20/MSEC );
					}
